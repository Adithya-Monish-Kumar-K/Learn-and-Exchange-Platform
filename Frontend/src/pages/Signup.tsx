...
const Signup: React.FC = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!/^\+?[0-9]{7,15}$/.test(form.phone.trim())) {
      setErr('Enter a valid phone number (7-15 digits)');
      return;
    }
    setLoading(true);
    try {
      console.log('[SIGNUP] Submitting register payload', {
        name: form.name,
        email: form.email,
        phone: form.phone,
      });
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        ...
        <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 text-center">
          ...
          <Link
            to="/login"
            className="inline-block w-full bg-blue-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-blue-700 transition mb-4"
          >
            Go to Login
          </Link>
